...
<template>
    <AuthenticatedLayout>
        <div class="mx-auto max-w-3xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div class="mb-6">
                <h1 class="text-2xl font-semibold text-slate-900 dark:text-white">Create Teacher</h1>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Add a new teacher account.</p>
            </div>
            ...
            <form class="space-y-5" @submit.prevent="submit">
                <div class="grid gap-5 md:grid-cols-2">
                    ...
                    <FormField label="Subject" for="specialization" :error="form.errors.specialization">
                    ...
                    <FormField label="Password" for="password" :error="form.errors.password" required>
                    ...
                    <FormField label="Confirm Password" for="password_confirmation" :error="form.errors.password_confirmation" required>
                ...
                <div class="flex justify-end gap-3">
                    <button type="button" @click="$inertia.visit(route('teachers.index'))" class="rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 dark:border-slate-700 dark:text-slate-300">Cancel</button>
                    <button type="submit" class="rounded-lg bg-amber-500 px-4 py-2 text-sm font-medium text-white hover:bg-amber-600">Save Teacher</button>
                </div>
            </form>
        </div>
    </AuthenticatedLayout>
